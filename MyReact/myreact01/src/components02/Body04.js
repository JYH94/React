...
function Viewer_Old(props) {
  console.log('** Old Child 컴포넌트 Update !');
  return (
    <div>
      <p>~ 여기는 Old Child 컴포넌트 ~!</p>
...
function Viewer({number}) {
  console.log('** New Child 컴포넌트 Update !');
  return (
    <div>
      <p>~ 여기는 New Child 컴포넌트 ~!</p>
...
function Body() { 
  ...
  return (
    <div className='body'>
      ...
      <Viewer number={10} />
      {/* 
      36번코드에서 number를 state변수로 지정했지만,
      자식컴포넌트에 전달해주는 number를 단순 상수로 지정해서 테스트 
      => 자식(Viewer)의 number 와 Body의 number은 다른 변수이니
      착각하지 말자
      => 렌더링은 
      */}

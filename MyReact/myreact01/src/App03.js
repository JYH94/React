...
function App() {
  ...
  useEffect(() => {
    console.log(`** useEffect Test2)count = ${count}, text = ${text}`);
  }, [count, text])
  ...
  //  -> 랜더링 할때마다 호출됨
  useEffect(() => {
    console.log(`** useEffect Test3 두번째 인자 없음 ) count = ${count}, text = ${text}`);
  })

  // => 4) LifeCycle 제어2: 두번째인자가 빈배열 인경우
  //  -> Mount 제어
  //  -> useEffect 에 빈 배열을 전달하면 마운트 시점에만 콜백함수 실행
  //    ( 처음 한번만 실행됨 확인 -> 그러므로 Mount 제어에 이용)
  useEffect(() => {
    console.log(`** useEffect Test4 빈배열 사용 ) count = ${count}, text = ${text}`);
  }, []);
  ...
  useEffect(() => {
    if (!didMountRef.current) {
      // 최초 렌더링(마운트) 시점 => 출력없이 return
      didMountRef.current = true;
    } else {
      console.log(`** useEffect Test5 리렌더링 ) count = ${count}, text = ${text}`);
    }
  });
  */
